...
fetch_direct() {
  ...
  if [[ -z "$url" ]]; then echo "No direct URL in $page" >&2; return 1; fi
  echo "direct $(basename "$dest") from $(printf '%s' "$url" | cut -c1-90)..."
  curl --http1.1 --fail --location --retry 4 --retry-delay 3 --connect-timeout 30 --max-time 7200 --user-agent "$UA" --output "$dest.part" "$url"
  mv -T "$dest.part" "$dest"
  local kind
  kind=$(file -b "$dest")
  echo "$(basename "$dest"): $kind"
  sha256sum "$dest" | tee -a "$OUT/SHA256SUMS.direct"
}
fetch_direct "$OUT/EA_SPORTS_FC.apk" "$OUT/EA_SPORTS_FC_MOBILE_BETA.apk"
fetch_direct "$OUT/obb.zip" "$OUT/obb.direct.zip"
fetch_direct "$OUT/Fifa16ModFC27.zip" "$OUT/Fifa16ModFC27.direct.zip"
